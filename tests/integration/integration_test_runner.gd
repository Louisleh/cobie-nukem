...
func _test_secret_counting_and_exit() -> void:
	...
	finale.interact(null)
	_expect(walker.golden_ball_strikes == 0, "Golden Ball reward never applies hidden boss damage")
	_expect(finale.claimed_once and not finale.enabled and not finale.visible, "Golden Ball exit condition can trigger only once and hides after collection")
	sign.free()
	wall.free()
	finale.free()
	walker.free()

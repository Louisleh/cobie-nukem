...
- **Current packet:** Saturday WCB-008K lighting-only review candidate on `codex/saturday-level1-opening-20260919`; **release/integration gate BLOCKED** by two ObjectDB leaks in the full-matrix smoke test and failed rendered frame-time budgets. Not a polished-level completion or WCB-009 advance.
...
- **Next dependency-safe packet:** isolate the two-object smoke-test leak before integrating the Saturday candidate; then scope a Salmon Creek enemy-label/readability and genuine opening-combat evidence packet. ENV-2B optional discovery remains parked, not a blocker to CLI work. Heavy renders still require restoring the 20 GiB reserve. No implicit Movie Maker repair or release promotion.

...
- **Current packet:** WCB-008K Salmon Creek Level-1 correctness repair on `codex/saturday-level1-opening-20260919`; **release/integration gate BLOCKED** by headless and rendered frame-time budgets. The smoke leak was not observed in ten repeated focused runs after cleanup repair, but needs continued full-matrix evidence. Not a polished-level completion or WCB-009 advance.
- **Latest candidate evidence:** actual Level 1 confirmed as Salmon Creek; source-bound five-second sampled native before/after at 1280×720 and 1024×768, focused lighting/probe tests, fresh Web/macOS exports and packaged-Chrome entry/movement/pause. See `docs/evidence/saturday-20260919/README.md` for exact results and remaining gates. ENV-2A remains the last completed environment packet.
- **Last verified work:** 2026-09-24 candidate-only Level-1 finale transaction and smoke teardown fixes: actual interaction/save-failure retry regression passes; ten repeated smoke runs are leak-clean; editor import/core contracts pass. Full non-export validation remains red on headless performance. ENV-2A bridge receipt remains `docs/evidence/environment-20260915/ENV-2A.md`; no gameplay art/device acceptance.
...
- **Next dependency-safe packet:** establish reproducible headless/native performance baselines under a safe disk reserve, then scope a Salmon Creek enemy-label/readability and genuine opening-combat evidence packet. ENV-2B optional discovery remains parked, not a blocker to CLI work. Heavy renders still require restoring the 20 GiB reserve. No implicit Movie Maker repair or release promotion.
...
## 2026-09-19 — Saturday Level 1 sprint / S1 REVIEW CANDIDATE, RELEASE BLOCKED

- Owner: Astra/Hermes; source `4311937d768a12de88693013aaed47c3d694de17`, clean existing buildout branch. Window verified locally at 12:01 PDT; no new work after 15:30, close by 16:00. This invocation owns canonical integration only.
- Identity reconciliation: `resources/campaign/episode_one.tres` orders Salmon Creek first; `resources/level/salmon_creek_card.tres` points to `scenes/levels/episode_1_level_1.tscn`. The Saturday priority is **Salmon Creek**, not Rain City. PRD §1.5 explicitly permits bounded opening child packets; Rain City remains the definitive slice. WCB-007 and WCB-008L M2–M5 remain frozen. No mission/weapon/enemy/progression breadth.
- Acceptance: obtain source-bound, bounded non-Movie-Maker first-level rendered evidence using ordinary named input and isolated saves, then select one evidence-backed opening presentation change, preserving collision/navigation and established human gates. A five-second probe precedes any longer run; sampled motion is not a human playthrough or canonical WCB-008L receipt.
- Owned paths: additive `tools/evidence/salmon_opening_probe.gd`, `tools/evidence/salmon_opening_probe.py`, focused wrapper tests, `docs/evidence/saturday-20260919/`, and this ledger. Production ownership will be frozen after pixel inspection, not guessed before baseline.
- Preflight: Godot 4.7.1, Blender 5.2.0 LTS, Material Maker 1.7 found; clean editor import exit 0. No active Godot/Blender/CLI writer found; existing stdio MCP processes left untouched. Optional Codex MCP discovery still fails; CLI fallback selected without spending the sprint on optional configuration. Health fails only missing Blender MCP and Codex MCP privacy/discovery; no bridge enabled in canonical source.
- Storage: 12 GiB available, above 6 GiB minimum but below 20 GiB heavy-workload reserve. No heavy renders or parallel engine workloads authorized; this packet limits capture scratch to 64 MiB and keeps an independent disk/process/time guard. Model downloads and unrelated caches remain untouched.
- Capacity: sanitized endpoint says allowed, 0% used / 100% remaining; reports reset September 26 12:01 PDT and omits weekly/Spark fields. This does **not** establish the expected 07:30 reset. Explicit Spark pilot reports HTTP 400 model unsupported for this ChatGPT account; no further Spark retries or silent model substitution. Parent continues independent safe work.
- Continuation: none scheduled. The enclosing existing-job instruction prohibits creating/updating schedules, so the nested continuation request is not acted on.
- Evidence/human boundary: mechanical and rendered-automated **review candidate**; not human/device approval. Final results below supersede this packet's initial pending state.
- **Validation repair ownership:** the first release attempt stopped on `world_interaction_test.gd`'s temporary-effect cleanup assertion; an unchanged isolated rerun passed, so it remains a disclosed intermittent failure (not silently weakened). The second attempt reached `umbrella_shield_content_test.gd:112` and raised a real previously-freed-instance error: its death timer may free the fixture before unconditional teardown. Parent additionally owns only that test's teardown guard; all damage/death/state assertions stay intact. No enemy runtime/balance change. The failed/hanging release attempt was stopped before retry. A later full retry reproduced the world-interaction cleanup failure; source inspection found a 0.18+0.22-second tween with a deferred free, observed by a 0.5-second SceneTree timer. Parent also owns that test's addition of two process-frame flushes after the unchanged timer; the exact baseline-count assertion and runtime tween remain unchanged. This is a lifecycle-order repair, not removal of a cleanup gate.
- **S1 production ownership transfer (12:08 PDT):** the baseline's field/structures merge into a teal-grey haze and its central threat/route stack is cluttered. This packet owns only `_build_lighting()` in `scripts/level/salmon_creek_world_builder.gd` plus an additive `tests/integration/salmon_creek_opening_lighting_test.gd`. Proposed reduction of fog/ambient wash and slightly stronger existing directional key must preserve exact lighting node count and every non-lighting byte in that production file. No assets, enemy HP-label code, encounter timing, collision, navigation, sign, shed layout, or other mission may change. Oversized enemy labels are recorded as an out-of-scope follow-up, not silently edited. Art brief: retain Salmon Creek wet evergreen/storm blue with the existing amber shed anchor and original municipal sign; improve field-to-shed depth/value separation without universal wet mirrors, new lights, or gameplay change. Source ownership transfer supersedes the old sign-only world-builder restriction for this **lighting-only** child packet; WCB-008K remains human/30-second-evidence open.

### S1 checkpoint results

- **Candidate branch:** `codex/saturday-level1-opening-20260919`. No merge into `hermes/world-class-369-buildout`, baseline approval, release stamp or deployment. Only independently exercised candidate work is preserved; program integration remains blocked.
- **Owned files delivered:** lighting-only function change, new lighting test, two test teardown repairs, additive five-second probe/wrapper and five Python tests, plus this ledger and `docs/evidence/saturday-20260919/`. Existing assets/provenance, campaign, runtime input/AI/combat/collision/navigation and export presets are unchanged.
- **Exact mechanical commands:** `bash tools/run_godot_safe.sh --timeout 600 -- --headless --path . --editor --quit` → exit 0; canonical `--script res://tests/run_tests.gd` with timeout 300 → `PASS: core contract checks`; focused `salmon_creek_opening_lighting_test.gd`, `salmon_creek_opening_presentation_test.gd`, repaired `umbrella_shield_content_test.gd` → PASS; repaired `world_interaction_test.gd` → three sequential PASS runs; `python3 -m unittest discover -s tools/tests -p test_salmon_opening_probe.py -v` → 5 tests PASS; content validation → five manifests PASS; architecture, asset/IP heuristic, docs validator and `git diff --check` PASS.
- **Exact rendered command:** `/Users/orion/.hermes/hermes-agent/venv/bin/python tools/evidence/salmon_opening_probe.py --out <fresh-output> --seconds 5 [--tablet]` → matched before/after at both supported probe aspects, 50 samples/run, measured 150 process frames and 300 physics ticks, verified actual user/save roots and matching start/end actor paths. Separate final Astra static re-review approves wrapper corrections and teardown edits. Model pixel review finds only modest visibility improvement; no major-polish claim.
- **Full gate failed:** `QA_EXPORTS=1 bash tools/release_validate.sh` reached 57 script invocations, including route/soak passes, then correctly rejected the smoke test's two ObjectDB leaks. Failed attempts preserved; no passing full-matrix claim.
- **Rendered performance failed:** bounded isolated `--resolution 1920x1080 --script res://tests/smoke/zone_performance_profile.gd` → exit 1; final opening average 19.576 ms / p95 57.979 ms / p99 71.375 ms. A first cache-directory setup error was fixed in the invocation, not production; the clean-setup rerun still misses budgets. No matched baseline timing was collected; no attribution or improvement claim.
- **Independent export checks:** safe-runner `--export-release Web builds/web/index.html` and `--export-release macOS builds/macos/CobieNukem.zip` → both exit 0 with no engine/script error lines; ZIP integrity and both PCK debug/bridge exclusions PASS. Exact sizes/hashes are in `packages.json`; Web PCK SHA-256 `62c200cdbbfcdbe007112604fd2ae3a18fb2c9dd80dfb04922c08e8e264299d8`.
- **Packaged Web:** isolated installed-Chrome/Playwright execution enters the real first level through menu/Doghouse/selector, moves, pauses and resizes; zero captured console warnings/errors/page errors. Browser 4:3 resize is letterboxed; touch gestures, full route/death/retry/completion, Safari and physical devices remain untested. No user Chrome/profile/config was changed.
...
## 2026-09-24 — WCB-008K Level-1 finale and smoke cleanup repair (release blocked)

- Source: `af4950a118a4daffe73ddef290ee0d0705682ac8`, clean candidate branch before edits; no program-branch integration or public deployment.
- Owner / paths: Hermes integration; `scripts/level/episode_1_level_1.gd`, `tests/integration/adversarial_state_test.gd`, `tests/smoke/smoke_test_runner.gd`, and this log. No boss, collision, navigation, art or balance change.
- Acceptance: the actual Golden Ball interaction commits a finished run exactly once and restores claimability if campaign persistence fails; smoke teardown releases the asynchronously routed scene/audio instead of accepting ObjectDB leaks.
- Diagnosis: `GoldenBallFinale.interact()` disables the ball before emitting `claimed`, while the Level-1 handler previously rejected a disabled ball. Direct handler calls in earlier tests missed the real interaction path. A missing SaveManager then left the ball consumed despite a retry message. The intermittent smoke leak reproduced in the read-only audit at 2/10 ordinary and 3/10 verbose runs; verbose ObjectDB named one `AudioStreamPlaybackWAV` and one `AudioStreamWAV`. The smoke fix explicitly frees the current routed scene, drains frames and audio teardown, and defers quit without allowlisting diagnostics.
- Exact verification: focused `res://tests/integration/adversarial_state_test.gd` exit 0 / `ADVERSARIAL STATE TESTS: PASS` with intentional missing-scene fixture warnings; focused smoke exit 0 / `PASS: 80 scenes and 189 resources`; ten additional serialized smoke runs each exited 0 with the sentinel and no `SCRIPT ERROR`, ObjectDB, resource, orphan-StringName or `ERROR:` diagnostics. Fresh editor import, core contracts, docs validator and `git diff --check` passed.
- Full non-export `bash tools/release_validate.sh` **FAILED** after the smoke test passed: five-mission `performance_smoke.gd` printed opening average 42.277 ms / p95 129.123 ms / p99 138.446 ms; the other four scenes also exceeded their headless budgets. This is a test failure, not a passing full matrix. A subsequent standalone headless profile printed similar opening average 45.678 ms / p95 127.799 ms / p99 138.678 ms and no PASS sentinel; its Godot process exited 0 despite missed budgets, demonstrating why the wrapper's output gate matters. The prior native 1920×1080 Salmon opening p95 57.979 ms / p99 71.375 ms remains failed and was not rerun. No new export, browser combat/death/completion, full human playthrough or device evidence is claimed.
- Resource safety: 4.1 GiB free at verification, below the workstation doctor's 6 GiB ordinary minimum and 20 GiB high-workload reserve. A stale, unopened 718 MiB Slack ShipIt stderr cache log was removed after an `lsof` owner check; active model/browser caches and other projects were retained. No heavy capture was launched.
- Review/next: independent read-only review of `fb182af` found one major transactional gap: the final objective was marked complete before save success. The handler now records that objective and victory narrative only after successful persistence; the actual-interaction regression asserts the objective remains incomplete on missing-service failure and completes on retry. Focused adversarial rerun passes. The fixture proves missing-service failure, not a disk-write fault. Retain candidate-only status; obtain repeated full-matrix evidence plus matched native performance before integration. Then prove ordinary-input Level-1 fire/contact/death/retry/finale, package/trace browser, and prepare named human art/feel/device review. Human taste, pacing, fairness, mix, humor, comfort, photosensitivity, Safari and physical device gates remain open.
- Integrated commit: candidate repair `fb182af`, evidence `561e31a`, reviewer correction `d91c44579cd2895edc573ca1c75d0f476fffc013` pushed and remote-ref verified; program branch none. A single next-packet continuation is scheduled for 2026-09-25 09:00 PDT (Hermes job `e40f118a031c`); it must recheck disk/concurrent writers, stop at one verified packet, and not schedule a chain.

## 2026-09-24 — Level-1 enemy readout candidate (visual gate open)

- Source: candidate branch `codex/saturday-level1-opening-20260919` at `3f0f5f8` before this packet; production/test commit `0210f19`. Owner: Hermes integration. Paths: `scripts/ai/enemy_agent.gd`, `scripts/ai/compliance_gull.gd`, `tests/unit/enemy_contract_tests.gd`, `tests/unit/compliance_gull_test.gd`. No damage, balance, HUD, collision, route or save changes.
- Presentation: keep health bars; hide undamaged numeric labels; briefly reveal smaller HP after a real hit at 2.6–10 m, with a 1.25-second refreshed window. Elite/boss actors (max HP ≥200) retain a 16 m named-readout range and a 2.5-second hit window. Point-blank and >24 m bars remain suppressed. No per-hit nodes/timers added.
- Independent read-only review flagged Gull's custom dive-physics bypass and elite/boss coverage. Follow-up in the same commit advances the readout during Gull dive/recovery without double-moving the actor and tests damaged Hound/Walker at 14 and 18 m. A first Gull regression failed because its synthetic scene lacked a camera; it was corrected with a real current camera, not skipped. Focused rerun: `enemy_contract_tests.gd` PASS, `compliance_gull_test.gd` PASS, `ui_scene_test.gd` PASS, `bash tools/architecture_check.sh` PASS, `git diff --check` clean. Editor import also passed before review. The node-count assertion is not a sustained memory/material budget proof.
- This is a **mechanical candidate only**, not rendered art acceptance, a full release pass, or an integrated/program-branch result. Native frame pacing and the five-mission performance gate remain red. At the time of coding, disk was 5.0 GiB free, below the 6 GiB capture floor; no heavy render or 30-second capture was run. Match near/full/hit/expiry/distance and multi-enemy frames across supported aspects after reserve is restored; obtain human readability judgment.
- Separate isolated harness writer owns `tools/evidence/salmon_opening_probe.gd`, `.py`, and `tools/tests/test_salmon_opening_probe.py` in scratch worktree `codex/cobie-opening-input-harness-20260924`; its result must be independently reviewed and merged only after it returns. Do not confuse its code/tests with an observed fired shot. The scheduled 2026-09-25 job must recheck this active writer and current branch before modifying anything.

## 2026-09-24 — Real short shot receipt and disk recovery (still release blocked)

- Input-harness candidate `b827ce3` cherry-picked from isolated worker `e3561e9`; Python validator tests 5/5 PASS, fresh headless editor import PASS. Independent review found no confirmed API mismatch, but `fire_input.events` observes submission, not the player's `_unhandled_input` callback. No direct activation, transform assignment, or weapon method invocation.
- After disk crossed the wrapper's 6 GiB minimum, native debug probes at 1280×720 and 1024×768 each passed: 150 measured process frames, 300 physics ticks, 50 validated images, real mapped mouse press/release at frames 90/93, one observed Pawstol `fired` signal, ammo 15→14. Desktop sampled player Z 10→-1.935. Source/harness digest and isolated save/user paths validated. A one-second preliminary probe also passed. Selected frames, structured reports and inspection limits are at `docs/evidence/opening-fire-20260924/README.md`; full 50-frame scratch sequences were validated and then removed. These prove a shot, **not enemy contact**, a 30-second combat run, packaged Web, or human feel.
- Pixel inspection: nonblank desktop and 4:3 frames; no obvious 4:3 HUD clipping, but the shed/field composition remains flat and sparse, HUD/portrait visually heavy. No AAA claim. Five-mission performance and native frame-time gates remain red.
- Storage: removed old downloaded Claude/Chrome `.dmg` installers after confirming installed apps and no mounted/open images, and removed regenerable `builds/web` and `builds/macos` outputs; existing export/package evidence cannot substitute for a fresh build. Removed two dormant Gemma 4 Hugging Face cached repos using `hf cache rm` after verifying no meshllm/Gemma process, no active Hermes fallback chain, and Ollama retained only the `nomic-embed-text` embedding alias. Managed deletion reported 13.1 GB decimal; data-volume free space rose from 6.1 to 18 GiB. Preserved active Docker VM, UV cache (native prune timed out waiting on live `uv run hyper-monitor` lock), current TTS/Whisper models and Ollama embedding. Do not force UV prune or wipe app data while services run. High-workload 20 GiB reserve is still unmet.

## 2026-09-24 — WCB-008K authored shed crest packet frozen (writer pending)

- Baseline `8b14b81` clean. Acceptance: a visibly authored pitched municipal-shed threshold improves the existing field-to-shed landmark at native 16:9 and 4:3 without changing collision, navigation, gate/trigger, objective, encounter, enemy scale, damage, lighting, HUD or save behavior; imported presentation kit remains eight opaque single-surface materials and zero CollisionObject3D nodes. Exact visual judgment and native/Web performance remain open until measured.
- Writer owns only `tools/blender/build_salmon_opening_foundry.py`, generated `assets/source/blender/salmon_creek_opening_foundry.blend`, `assets/models/environment/salmon_creek_opening_foundry.glb`, `tests/unit/asset_contract_test.gd`, and the exact three source/runtime entries in `docs/ASSET_MANIFEST.md`. Reuse existing palette and coordinate contract; no new material/light/transparent surface. Target original shed-front silhouette above existing roof, rather than another fog or HUD-only adjustment. Independent owner retains this log, capture tools/evidence, integration, review and release claims.
- Gate: deterministic Blender rebuild → Godot 4.7 import → asset and opening-presentation contracts → canonical core suite and architecture/content checks → source-bound matched desktop/4:3 field and shed captures, draw-call/frame-time comparisons and fresh packaged Web validation. Do not infer a release-quality transformation from a successful GLB import. Prior rendered opening p95 57.979 ms and five-mission headless p95 120–150 ms remain failed. Optional Blender/Codex MCP health currently fails; CLI Blender/Godot paths and core import/contracts work. Disk 18 GiB, below 20 GiB high-workload reserve; bound every heavy run.
- Four independent read-only audits at this source also proposed a separate continuous contact harness, diagnostic-only pacing instrumentation and a HUD caption/audio pass. These are sequenced after the shed packet; no concurrent writer owns their paths. A five-second input-driven shot is already real, but enemy contact, 30 seconds, human art/feel and physical-device evidence remain open.

## 2026-09-25 — WCB-008K performance preflight (diagnostic-only; release blocked)

- **Source/ownership:** candidate `de39588cb926ad72ab375497d17d4a49bfd2ecdb` on `codex/saturday-level1-opening-20260919`; Hermes owns only this ledger entry. The branch is seven commits beyond the requested `d91c445` baseline and one commit ahead of its origin; the isolated shed-crest branch at `27582af` remains a separate, unintegrated candidate. No active game/Blender/Codex writer was observed; the long-lived Godot MCP stdio process is not an active editor. No gameplay or test gate changed.
- **Acceptance/evidence:** classify the failed five-mission headless p95 before optimizing a Level-1 scene; use bounded, non-capture probes at available disk headroom. `df -h .` reported 17 GiB free, above the 6 GiB ordinary minimum but below the 20 GiB heavy-workload reserve. No task-owned cache large enough to safely close that gap was confirmed, so nothing was deleted and no render/export/capture was launched.
- **Exact commands/results (mechanical, not rendered):** `bash tools/game_dev_health.sh` reported Godot 4.7.1 and clean source/idle bridge, but exited 1 for missing optional Blender/Codex MCP setup. `bash tools/run_godot_safe.sh --timeout 180 -- --headless --path . --script res://tests/smoke/performance_smoke.gd` exited 1: Salmon Creek average 43.655 / p50 20.122 / p95 124.236 / p99 136.626 ms; Rain City p95 124.766, Mount Hood 147.826, Moon 147.645, Ventura 145.197 ms. A 300-frame empty `SceneTree` probe stored only in task scratch, run through the same wrapper/project, exited 0 with p50 19.670 / p95 148.582 / p99 149.017 ms. The same empty probe in a minimal scratch Godot project gave p95 61.370 ms. The in-project empty probe with `--fixed-fps 60` gave p95 0.003 ms; the full smoke with that diagnostic-only switch exited 0 with Salmon p95 0.197 ms, but forced fixed FPS disables real-time synchronization, so this is **not** a substitute release gate or rendered performance pass.
- **Conclusion/remaining gates:** the roughly 125–150 ms headless p95 cannot be attributed to Salmon Creek scene work alone; even the empty frame loop shows major scheduling/idle variance. Cause of the host/project timing gap is not yet isolated. The previously measured native rendered opening p95 57.979 ms is a distinct failed gate and remains unaddressed. Do not weaken the headless budget or promote the fixed-FPS result. No new import/core/release suite, fresh export, human/device test, or shed visual review was run in this diagnostic packet.
- **Next dependency-safe packet:** isolate the host versus project frame-scheduler/autoload contribution with bounded instrumentation, then obtain matched rendered Level-1 CPU/GPU/draw-call evidence after restoring 20 GiB reserve. Only then review/integrate the unmerged shed-crest candidate or make an evidence-backed smallest production optimization. Human art, feel, humor, accessibility, Safari and physical-device gates remain open; no program-branch integration or release claim.

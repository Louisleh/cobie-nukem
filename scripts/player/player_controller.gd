...
func apply_mission_loadout(profile: MissionLoadoutProfile, restored_payload: Dictionary = {}) -> bool:
	if profile == null or not profile.validate().is_empty() or weapons.is_empty():
		return false
	...
	if selected_index < 0 or not weapons[selected_index].unlocked:
		for index in weapons.size():
	...
	if selected_index < 0:
		return false
	select_weapon(selected_index)
	return true


func mission_loadout_snapshot(mission_id: StringName, mission_upgrades: Array[StringName] = []) -> Dictionary:
	...
	var selected_id := &""
	if current_weapon_index >= 0 and current_weapon_index < weapons.size() and weapons[current_weapon_index].definition != null:
		selected_id = weapons[current_weapon_index].definition.id

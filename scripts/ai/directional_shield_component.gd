...
func reset() -> void:
	...
	shield_active = maximum_shield_health > 0.0
	_is_broken_once = false
...
func damage_multiplier(owner_body: Node3D, hit_position: Vector3, _incoming_damage: float = 0.0) -> float:
	...
	if _is_shield_hit(body, hit_position):
		...
		if current_shield_health <= 0.0 and not _is_broken_once:
			shield_active = false
			_is_broken_once = true
			if _shield_visual != null:
